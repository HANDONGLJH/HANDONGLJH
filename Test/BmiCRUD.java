...
public class BmiCRUD implements iCRUD {
  ...
  public void loadData() throws IOException {
    File file = new File("/Users/jaeho/Documents/LJH/test/resource/data.txt");
  ...
  @Override
  public int report() {
    HashMap<String, ArrayList<Double>> levels = new HashMap<>();
    for (BmiItem item : list) {
      levels.putIfAbsent(item.getLevel(), new ArrayList<>());
      levels.get(item.getLevel()).add(item.getBmi());
    }
    for (Map.Entry<String, ArrayList<Double>> entry : levels.entrySet()) {
      double average = entry.getValue().stream().mapToDouble(a -> a).average().orElse(0.0);
      System.out.printf("Level: %s, Count: %d, Average BMI: %.2f\n", entry.getKey(), entry.getValue().size(), average);
    }
    return 0;
  }
